...
async function extractString(blockText) {
  ...
  if (match) {
    ...
    if (
      ...
      console.warn(
        'QueryBuilder API is not available on window.roamjs.extension.queryBuilder',
      );
    }
  }
...
const parseModulesFromString = async (rawString: string) => {
  const output: any = [];
...
const loadContext = async (tree: any): Promise<any[]> => {
  const treeRaw = parseRoamTree(tree.children);
  const blocksToLoad = await parseModulesFromString(treeRaw);

  let contextPackets: any = [];

  blocksToLoad.map((block: any) => {
    let blockUid = block.id;
    let title = block.title;
    let blockContext: string = '';

    if (block.type === 'page') {
      blockUid = getPageUidByPageTitle(block.title);
    }
    ...
    contextPackets.push({ role: 'user', content: blockContext });
  });

  return contextPackets;
};
...
const sendRequest = async (option: any, model: any) => {
  ...
  if (option?.outputType === 'chat') {
    ...
    if (option?.loadPages === true) {
      const contextPackets = await loadContext(tree);
  ...
  // replace the "qq" text
  updateBlock({
    text: getTextByBlockUid(lastEditedBlockUid).replace(
      new RegExp(' qq$'),
      ` ${CONTENT_TAG}`,
    ),
  ...
  fetch(url, {
    ...
    .then((data) => {
      if (data.error) {
        ...
        createBlock({
          node: { text: `[assistant]: ${data.error.message}` },
      ...
      settings: [
        ...
        },
        {
          action: {
            ...
            placeholder:
              '[{ "name": "model A", "endpoint": "closedai.com/v1/completions", "displayName": "model A" }]',
          },
          id: 'custom_models',
          name: 'Custom models',
